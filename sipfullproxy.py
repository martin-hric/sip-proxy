...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 FUNGUJE 0K")
                return
        else:
            now = int(time.time())
        ...
        logs.write(time.strftime("%d %b %Y %H:%M:%S ", time.localtime()))
        logs.write("Registracia noveho uzivatela[%s]\n" % fromm)
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            self.sendResponse("500 SERVEROVY ERROR")
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 NEPRIJATY")
        else:
            self.sendResponse("500 SERVEROVY ERROR")
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                if self.data[0] == 'SIP/2.0 100 Trying':
                    data[0] = 'SIP/2.0 100 Skusam Vytocit'
                elif self.data[0] == 'SIP/2.0 180 Ringing':
                    data[0] ='SIP/2.0 180 Zvoniiim'
                elif self.data[0] == 'SIP/2.0 200 OK':
                    data[0] = 'SIP/2.0 200 Poriesene OK'
                    if data[7] == 'CSeq: 2 BYE':
                        logs.write(time.strftime("%d %b %Y %H:%M:%S ", time.localtime()))
                        logs.write("[Hovor ukonceny] => ")
                        logs.write(f"{data[6]} || {data[5]} => {data[4]}\n")
                    elif data[7] == 'CSeq: 1 INVITE':
                        logs.write(time.strftime("%d %b %Y %H:%M:%S ", time.localtime()))
                        logs.write("[Hovor prijaty]  => ")
                        logs.write(f"{data[6]} || {data[5]} => {data[4]}\n")
                elif self.data[0] == 'SIP/2.0 487 Request Terminated':
                    data[0] = 'SIP/2.0 487 Ziadost Zrusena'
                    logs.write(time.strftime("%d %b %Y %H:%M:%S ", time.localtime()))
                    logs.write("[Ziadost zrusena] => ")
                    logs.write(f"{data[4]} || {data[3]} => {data[2]}\n")
                elif self.data[0] == 'SIP/2.0 486 Busy Here':
                    data[0] = 'SIP/2.0 486 Momentalne Zaneprazdneny'
                    logs.write(time.strftime("%d %b %Y %H:%M:%S ", time.localtime()))
                    logs.write("[Uzivatel zaneprazdneny] => ")
                    logs.write(f"{data[4]} || {data[3]} => {data[2]}\n")
